...
Look up Hebrew city names in a `cities.json` file by matching the `name` field.

**Primary source:** `cf-nodejs-apps/map-update-requests/generated/cities.json` (in the melameg/cf-nodejs-apps repo). This file is maintained by the live MUR app and accumulates cities organically from real Waze API responses — it is more complete and includes deep Negev settlements that may be absent from the grid-sweep file.

**Fallback source:** `wme-objects-info/cities.json` (in this repo). Generated by a manual grid-sweep script over a fixed geographic bounding box — cities at the edge or outside the grid (e.g. deep Negev) may be missing.

**Coordinates format differs between the two files:**
...
If an exact match fails, try a partial/fuzzy match (e.g. `חצור אשדוד` is stored as `חצור-אשדוד`). City IDs can also be extracted directly from Waze editor URLs (`citys=<id>`).
